...
import {
  cellDraggingClassname,
  cellOverClassname,
  cellResizableClassname,
  cellSortableClassname,
  dragImageClassname,
...
export default function HeaderCell<R, SR>({
  ...
  const { tabIndex, childTabIndex, onFocus } = useRovingTabIndex(
    shouldFocusGrid || isCellSelected,
  );
  ...
  const priority =
    sortColumn !== undefined && (sortColumns?.length ?? 0) > 1
      ? sortIndex! + 1
      : undefined;
  const ariaSort =
...
function ResizeHandle<R, SR>({
  ...
  function onPointerDown(event: React.PointerEvent<HTMLDivElement>) {
    ...
    resizingOffsetRef.current = isRtl
      ? event.clientX - left
      : right - event.clientX;
  }
  ...
  function onPointerMove(event: React.PointerEvent<HTMLDivElement>) {
    ...
    const { width, right, left } = parentElement.getBoundingClientRect();
    let newWidth = isRtl
