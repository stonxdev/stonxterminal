...
export function TreeDataGrid<R, SR = unknown, K extends Key = Key>({
  ...
  const { columns, groupBy } = useMemo(() => {
    const columns = [...rawColumns].sort(({ key: aKey }, { key: bKey }) => {
      ...
      if (rawGroupBy.includes(aKey as string)) {
        if (rawGroupBy.includes(bKey as string)) {
          return (
            rawGroupBy.indexOf(aKey as string) -
            rawGroupBy.indexOf(bKey as string)
          );
        }
        return -1;
      }
      if (rawGroupBy.includes(bKey as string)) return 1;
  ...
  const [rows, isGroupRow] = useMemo((): [
    ...
    const expandGroup = (
      ...
      Object.keys(rows).forEach((groupKey, posInSet, keys) => {
        ...
        const groupData = rows[groupKey];
        if (!groupData) return;
        const { childRows, childGroups, startRowIndex } = groupData;
  ...
  function handleCellPaste(
    ...
    if (isGroupRow(row)) return row as NoInfer<R>;
    const result = rawOnCellPaste?.({ row, column }, event);
    return result as NoInfer<R>;
  }
  ...
  function handleRowsChange(
    ...
    for (const index of indexes) {
      ...
      if (updatedRow && rawIndex !== -1) {
        updatedRawRows[rawIndex] = updatedRow;
      }
