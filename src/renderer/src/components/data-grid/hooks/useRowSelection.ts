...
export function useHeaderRowSelection() {
  ...
  const headerRowSelectionChangeContext = useContext(
    HeaderRowSelectionChangeContext,
  );

  if (
    headerRowSelectionContext === undefined ||
    headerRowSelectionChangeContext === undefined
  ) {
    throw new Error(
      "useHeaderRowSelection must be used within renderHeaderCell",
    );
  }
