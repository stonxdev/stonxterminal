...
export function useCalculatedColumns<R, SR>({
  ...
  const defaultRenderHeaderCell =
    defaultColumnOptions?.renderHeaderCell ?? renderHeaderCell;
      ...
      function collectColumns(
        ...
        for (const rawColumn of rawColumns) {
          ...
          const column: MutableCalculatedColumn<R, SR> = {
            ...
            renderHeaderCell:
              rawColumn.renderHeaderCell ?? defaultRenderHeaderCell,
          };
  ...
  const {
    ...
    if (lastFrozenColumnIndex !== -1) {
      ...
      if (lastFrozenColumn) {
        const columnMetric = columnMetrics.get(lastFrozenColumn)!;
  ...
  const [colOverscanStartIdx, colOverscanEndIdx] = useMemo((): [
    ...
    while (colVisibleStartIdx < lastColIdx) {
      const column = columns[colVisibleStartIdx];
      if (!column) break;
    ...
    while (colVisibleEndIdx < lastColIdx) {
      const column = columns[colVisibleEndIdx];
      if (!column) break;
      const { left, width } = columnMetrics.get(column)!;
